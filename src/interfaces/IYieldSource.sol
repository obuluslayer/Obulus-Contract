...
/// @author Obulus
/// @notice A yield source is an OPT-IN, swappable backend the `ObulusYieldVault` may route its idle USDG to. The
///         vault holds the depositors' shares; the source holds (some or all of) the underlying USDG and
///         tries to grow it. The vault always values itself by `totalAssets()` read back from the source plus
///         whatever idle USDG it still holds directly, so share price reflects realized gains AND realized
///         losses honestly.
///
...
///    (e.g. the Uniswap v4 adapter) exposes deposited USDG to impermanent loss, slippage and pool
...
///    vault is a pure pass-through holding USDG directly), or (b) a source that is itself principal-protected
...
///  - `deposit(assets)` is called by the vault, which has ALREADY transferred `assets` USDG to the source
...
///  - `withdraw(assets, to)` instructs the source to return up to `assets` USDG to `to` (the vault). It
...
///  - `totalAssets()` is the source's best honest estimate of the USDG currently recoverable from it,
...
interface IYieldSource {
    ...
    /// @notice Notify the source that `assets` USDG have just been transferred to it by the vault and should
    ///         be put to work. Caller is the vault. The source MUST NOT pull tokens from the caller.
    /// @param assets The amount of USDG already delivered to this source.
    function deposit(uint256 assets) external;

    /// @notice Return up to `assets` USDG to `to`. For a lossy/illiquid source the amount actually sent MAY
    ...
    /// @return sent  The amount of USDG actually transferred to `to` (<= assets).
    function withdraw(uint256 assets, address to) external returns (uint256 sent);

    /// @notice The source's honest valuation of the USDG currently recoverable from it (principal +
