...
///         touched. It reads the SETTLEMENT TOKEN (`usdg`) from the ALREADY-WRITTEN
...
///         (`escrow`, `usdg`, `chainId`, `deployBlock`, and any `stakingVault`/`yieldVault`) so all current
...
///
/// ObulusSubscriptionEscrow constructor is `(usdg, treasury, feeBps, arbFeeBps, resolveTimeout)` — the same
...
///   USDG_ADDRESS         override the settlement token (USDC_ADDRESS still accepted); else read `usdg` from deployments/<chainId>.json
...
contract DeploySubscription is Script {
    ...
    struct SubConfig {
        address usdg; // settlement token (read from deployments/<chainId>.json, or USDC_ADDRESS override)
    ...
    function _deployFrom(SubConfig memory cfg) internal returns (address subscriptionEscrow) {
        ...
        ObulusSubscriptionEscrow sub =
            new ObulusSubscriptionEscrow(cfg.usdg, cfg.treasury, cfg.feeBps, cfg.arbFeeBps, cfg.resolveTimeout);
    ...
    /// @dev USDC_ADDRESS env override, else the `usdg` field of deployments/<chainId>.json (written by
    ...
    function _resolveUsdc() internal view returns (address) {
        ...
        require(vm.keyExistsJson(json, ".usdg"), "DeploySubscription: deployments file missing `usdg` key");
        return vm.parseJsonAddress(json, ".usdg");
    }
    ...
    ///      field so current readers (escrow/usdg/chainId/deployBlock/stakingVault/yieldVault) are untouched.
    ///      If it does not exist (USDC was supplied by env), we create it carrying usdg + the new key.
